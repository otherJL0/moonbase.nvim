...
local function bind(lib)
    if not C then
        if not lib or lib == 'mysql' then
            ...
            C = ffi.load('/usr/local/mysql/lib/libmysqlclient.dylib')
...
local function bool_ptr(b) return ffi.new('my_bool[1]', b or false) end

local function uint_bool_ptr(b) return ffi.new('uint32_t[1]', b or false) end

local function uint_ptr(i) return ffi.new('uint32_t[1]', i) end

local function proto_ptr(proto) -- proto is 'MYSQL_PROTOCOL_*' or mysql.C.MYSQL_PROTOCOL_*
    return ffi.new('uint32_t[1]', enum(proto))
end

local function ignore_arg() return nil end
...
function M.connect(t, ...)
    ...
    if type(t) == 'string' then
        ...
        host, user, pass, db, charset, port = t.host, t.user, t.pass, t.db,
                                              t.charset, t.port
        unix_socket, flags, options, attrs = t.unix_socket, t.flags, t.options,
                                             t.attrs
    ...
    elseif flags then
        for k, v in pairs(flags) do
            ...
            client_flag = v and bit.bor(client_flag, flag) or
                              bit.band(client_flag, bit.bnot(flag))
        end
    end
    ...
    if options then
        for k, v in pairs(options) do
            ...
            assert(C.mysql_options(mysql, opt, ffi.cast('const void*', v)) == 0,
                   'invalid option')
        end
    end
    ...
    if attrs then
        for k, v in pairs(attrs) do
            assert(
                C.mysql_options4(mysql, C.MYSQL_OPT_CONNECT_ATTR_ADD, k, v) == 0)
        end
    end
    ...
    checkh(mysql,
           C.mysql_real_connect(mysql, host, user, pass, db, port, unix_socket,
                                client_flag))
...
function conn.select_db(mysql, db) checkz(mysql, C.mysql_select_db(mysql, db)) end
...
function conn.set_multiple_statements(mysql, yes)
    checkz(mysql,
           C.mysql_set_server_option(mysql,
                                     yes and C.MYSQL_OPTION_MULTI_STATEMENTS_ON or
                                         C.MYSQL_OPTION_MULTI_STATEMENTS_OFF))
end

-- connection info

function conn.charset(mysql) return cstring(C.mysql_character_set_name(mysql)) end
...
function conn.stat(mysql) return cstring(checkh(mysql, C.mysql_stat(mysql))) end
...
function conn.proto_info(...) return C.mysql_get_proto_info(...) end

function conn.ssl_cipher(mysql) return cstring(C.mysql_get_ssl_cipher(mysql)) end
...
function conn.field_count(...) return C.mysql_field_count(...) end
...
function conn.sqlstate(mysql) return cstring(C.mysql_sqlstate(mysql)) end

function conn.warning_count(...) return C.mysql_warning_count(...) end

function conn.info(mysql) return cstring(C.mysql_info(mysql)) end
...
function conn.more_results(mysql) return C.mysql_more_results(mysql) == 1 end
...
function res.row_count(res) return tonumber(C.mysql_num_rows(res)) end

function res.field_count(...) return C.mysql_num_fields(...) end

function res.eof(res) return C.mysql_eof(res) ~= 0 end
...
local function field_type_name(info)
    ...
    field_type = info.charsetnr == 63 and binary_field_type_names[type_flag] or
                     field_type
    return field_type
end
...
local function parse_double(data, sz) return ffi.C.strtod(data, nil) end

ffi.cdef('int64_t strtoll(const char*, char**, int) ' ..
             (ffi.os == 'Windows' and ' asm("_strtoi64")' or '') .. ';')
local function parse_int64(data, sz) return ffi.C.strtoll(data, nil, 10) end

ffi.cdef('uint64_t strtoull(const char*, char**, int) ' ..
             (ffi.os == 'Windows' and ' asm("_strtoui64")' or '') .. ';')
local function parse_uint64(data, sz) return ffi.C.strtoull(data, nil, 10) end
...
local function parse_bit(data, sz)
    ...
    for i = 1, sz - 1 do n = n * 256 + data[i] end
    return n
end
...
local function parse_date_(data, sz)
    ...
    local z = ('0'):byte()
    local year =
        (data[0] - z) * 1000 + (data[1] - z) * 100 + (data[2] - z) * 10 +
            (data[3] - z)
...
local function parse_time_(data, sz)
    ...
    local frac = 0
    for i = 9, sz - 1 do frac = frac * 10 + (data[i] - z) end
...
local function datetime_tostring(t)
    local date, time
    if t.year then date = format_date(t.year, t.month, t.day) end
    if t.sec then time = format_time(t.hour, t.min, t.sec, t.frac) end
...
local function datetime(t) return setmetatable(t, datetime_meta) end
...
local field_decoders =
    { -- other field types not present here are returned as strings, unparsed
...
local unsigned_decoders = {[ffi.C.MYSQL_TYPE_LONGLONG] = parse_uint64}
    ...
    for i = 0, field_count - 1 do
        ...
        if v ~= NULL then
            ...
            if decode then
                ...
                local unsigned =
                    bit.bor(fields[i].flags, C.MYSQL_UNSIGNED_FLAG) ~= 0
                decoder = unsigned and unsigned_decoders[ftype] or
                              field_decoders[ftype] or ffi.string
            else
                decoder = ffi.string
            end
            v = decoder(values[i], tonumber(sizes[i]))
            if numeric then t[i + 1] = v end
            if assoc then
...
function res.fetch(res, mode, t)
    ...
    local row = fetch_row(res, numeric, assoc, decode, field_count, fields,
                          t or {})
...
function res.tell(...) return C.mysql_row_tell(...) end
...
function conn.kill(mysql, pid) checkz(mysql, C.mysql_kill(mysql, pid)) end

function conn.shutdown(mysql, level)
    checkz(mysql, C.mysql_shutdown(mysql, enum(
                                       level or C.MYSQL_SHUTDOWN_DEFAULT,
                                       'MYSQL_')))
end
...
function conn.refresh(mysql, t) -- options are 'REFRESH_*' or mysql.C.MYSQL_REFRESH_* enums
    ...
    if type(t) == 'number' then
        ...
        for k, v in pairs(t) do
            if v then options = bit.bor(options, enum(k, 'MYSQL_')) end
        end
    end
...
function stmt.exec(stmt) stcheckz(stmt, C.mysql_stmt_execute(stmt)) end
...
function stmt.store_result(stmt) stcheckz(stmt, C.mysql_stmt_store_result(stmt)) end

function stmt.free_result(stmt) stcheckbool(stmt, C.mysql_stmt_free_result(stmt)) end

function stmt.row_count(stmt) return tonumber(C.mysql_stmt_num_rows(stmt)) end
...
function stmt.insert_id(...) return C.mysql_stmt_insert_id(...) end

function stmt.field_count(stmt) return tonumber(C.mysql_stmt_field_count(stmt)) end

function stmt.param_count(stmt) return tonumber(C.mysql_stmt_param_count(stmt)) end
...
function stmt.sqlstate(stmt) return cstring(C.mysql_stmt_sqlstate(stmt)) end
...
function stmt.reset(stmt) stcheckz(stmt, C.mysql_stmt_reset(stmt)) end

function stmt.tell(...) return C.mysql_stmt_row_tell(...) end
...
function stmt.write(stmt, param_number, data, size)
    stcheckz(stmt, C.mysql_stmt_send_long_data(stmt, param_number, data,
                                               size or #data))
end
...
function stmt.update_max_length(stmt)
    ...
    stcheckz(stmt,
             C.mysql_stmt_attr_get(stmt, C.STMT_ATTR_UPDATE_MAX_LENGTH, attr))
    return attr[0] == 1
end
...
local function bind_buffer(bb_types, meta, types)
    ...
    for i, typedef in ipairs(types) do
        ...
        elseif number_types[btype] then
            ...
            data =
                ffi.new(unsigned and uint_types[btype] or number_types[btype])
...
function params:realloc(i, size)
    ...
    assert(ffi.istype(self.data[i], 'uint8_t[?]'),
           'attempt to realloc a fixed size field')
...
function fields:get_date(i)
    ...
    local date = btype == C.MYSQL_TYPE_DATE or btype == C.MYSQL_TYPE_DATETIME or
                     btype == C.MYSQL_TYPE_TIMESTAMP
    local time = btype == C.MYSQL_TYPE_TIME or btype == C.MYSQL_TYPE_DATETIME or
                     btype == C.MYSQL_TYPE_TIMESTAMP
    assert(date or time, 'not a date/time type')
    if self.null_flags[i - 1] == 1 then return nil end
    local tm = self.data[i]
    return date and tm.year or nil, date and tm.month or nil,
           date and tm.day or nil, time and tm.hour or nil,
           time and tm.minute or nil, time and tm.second or nil,
...
function fields:get(i)
    ...
    elseif time_types[btype] then
        ...
        local sz = math.min(tonumber(self.buffer[i - 1].buffer_length),
                            tonumber(self.lengths[i - 1]))
